...
function initializeApp() {
    ...
    // Event Listener für Neuestes/Ältestes Item
    const showNewestBtn = document.getElementById('showNewestBtn');
    const showOldestBtn = document.getElementById('showOldestBtn');

    showNewestBtn?.addEventListener('click', async () => {
        await showSpecialItem('newest');
    });

    showOldestBtn?.addEventListener('click', async () => {
        await showSpecialItem('oldest');
    });
    ...
    async function performSearch(searchTerm) {
        try {
            ...
            const res = await fetch(`${API}/search?name=${encodeURIComponent(searchTerm)}`);
            if (!res.ok) throw new Error('Suchfehler');
            
            const items = await res.json();
            
            if (items.length === 0) {
                searchResultInfo.textContent = `Keine Items gefunden für "${searchTerm}"`;
                searchResultInfo.style.color = '#d9534f';
            } else {
                searchResultInfo.textContent = `${items.length} Item(s) gefunden für "${searchTerm}"`;
                searchResultInfo.style.color = '#28a745';
            }
            
            renderTable(items);
        } catch (err) {
            console.error(err);
            searchResultInfo.textContent = 'Fehler bei der Suche';
            searchResultInfo.style.color = '#d9534f';
        }
    }
    ...
    async function exportToExcel() {
        try {
            ...
            // Hole den Dateinamen aus dem Header
            const contentDisposition = res.headers.get('Content-Disposition');
            let fileName = 'QR-Inventar_Export.xlsx';
            
            if (contentDisposition) {
                // Versuche zuerst mit Anführungszeichen
   ...
   async function fillSelects(input)
   {
    ...
    async function showSpecialItem(type) 
    {
        try 
        {
            const endpoint = type === 'newest' ? '/items/newest' : '/items/oldest';
            const res = await fetch(endpoint);
            
            if (res.status === 404) 
            {
                alert('Keine Items vorhanden.');
                return;
            }
            
            if (!res.ok) 
            {
                throw new Error('Fehler beim Laden des Items');
            }
            
            const item = await res.json();
            
            // Zeige das Item in einer Alert-Box oder scrolle zur Tabelle
            
            // Optional: Scrolle zur Tabelle und markiere das Item
        ...
        {
            console.error(err);
            alert('Fehler beim Laden des Items: ' + err.message);
        }
    }
}
